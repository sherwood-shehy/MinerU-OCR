...
- **减少目录误报**：结合可见文字、点线、页码对齐及顺序识别原生目录，支持无标题续目录和被模型拆成多个候选框的目录。
- **限定排除范围**：只排除已核实目录块内的候选框，并记录依据；图片、线框及同页其他表格保持原分流规则，不新增依赖或 OCR 服务调用。
- **真实文件复测**：GB/T 33349—2016 的两页目录不再误判，预检在物理第 8 页正确因合并单元格表格选择云端。详见 [0.4.3 验收记录](docs/v0.4.3-validation.md)。

## 0.4.2 的变化

- **表格风险前置分流**：文字层可靠、表格结构明确时优先本地；合并／不规则表格、图片表格和结构不确定的疑似表格使整份 PDF 走云端。普通插图本身不触发云端。
- **复用已有依赖**：使用固定版本 Layout 的图像版面模型和原生网格几何检测，补充无边框图片表格的疑似检测，不新增依赖或本地 OCR。预检离线；依赖或运行错误停止，不隐式上传。
- **简化后置验收**：复用预检保存的简单表格单元格，不重复检测或重建复杂合并矩阵；保留页序、文字、数字和技术符号检查。基本完整性失败时自动模式可回退云端，纯本地模式从不上传。
- **简单表格更易转 Markdown**：全 td 的规则表格允许按完整文字首行作为表头，并记录此约定；列数多、文字长不再视为复杂。空白／纯数字等不明确表头仍需确认，合并、多层表头、富内容等保留 HTML。
- **默认交付干净**：仅 MD 与必要正文图片，使用相对链接、同字节去重；不附加整页 PDF 截图和密集核对链接，完整原页图库需显式选择阅读版。
- **后处理保持边界**：只整理格式、执行明确校勘，不猜测丢失的表格关系；预检存在漏检和误判可能，云端结果也需复杂表格、公式等重点抽查。
...
- **通用源材料版成为默认模式**。`readable` 默认 `source + auto`：保守整理结构、选择性转换简单表格，不生成整页 PDF 截图，不向条款、表格和公式插入密集核对链接。`images/` 只存正文所需资源；完整原页图库须显式选择 `--edition reading`。
...
核心依赖为 httpx、pydantic、pypdf、platformdirs、python-dotenv；PDF 后处理使用 PyMuPDF。完整表格预检与自动分流需要 `local` 依赖组；该组固定已验证的 PyMuPDF、PyMuPDF4LLM、Layout 1.28.2，由 pip 安装其必要传递依赖。无需额外的本地 OCR 引擎、大模型接口或另一套表格解析库。标准库随 Python 提供，第三方库安装到经授权的现有 Python 环境，不打包进 skill 文件夹。若命令不在 PATH，可使用 `python -m mineru_ocr.cli`。
...
预检结合可见文字、点线和对齐且顺序合理的页码识别原生目录，支持续目录及模型将目录拆成多个候选框的情况。排除依据记录在 `tables.non_table_regions`，同页其他表格仍独立判断；缺少点线、条目不明确、含图片或表格线时继续保守处理。详见[分流规则](docs/readable-workflow.md)。
...
正文图片采用 `![图 1](images/<hash>.png)`。原页定位保留在处理记录中，默认交付不复制整页 PDF 截图；仅显式 `--edition reading` 生成原页图库及其核对链接。复杂表格仍使用 HTML，公式仍保留原格式，实际显示取决于阅读器的 HTML/数学公式支持。

## 保真与图片处理原则

`--table-format auto` 转换具有明确表头、经原页核对表头或完整文字首行的规则二维表；全 td 时采用首行为表头的约定并记录依据，不等于语义识别。列数和文字长度不作为复杂度标准；逐单元格往返检查文本、空格归一、空值、单位和符号。合并单元格、多层表头、复杂内容等保留 HTML。`--table-format html` 可保留所有 HTML 表格。
...
0.4.3 通过 **166 项离线测试**，其中新增 19 项目录回归，包含固定版 Layout 模型实测；原文件仅重新执行本地预检。详见 [0.4.3 验收记录](docs/v0.4.3-validation.md)。

0.4.2 通过 **147 项离线测试**，并对 10 份真实 PDF（400 页）进行预检。另一个本地候选在提取后因上下标／符号问题被拦截；本轮未调用真实云端 OCR。实测与限制见 [0.4.2 验收记录](docs/v0.4.2-validation.md)。
...
0.4.0 基线通过 **93 项离线测试**。复用 GB 6932—2015 的 108 页扫描 OCR，交付 1 份 MD 和 131 张引用图像；8 张表转为 Markdown、48 张保留 HTML。独立 GFM 渲染核对了全部 8 张转换表的 93 个单元格，素材包迁移后引用检查通过。详见[0.4.0 验收记录](docs/v0.4.0-validation.md)；本次新增验证见 [0.4.3 验收记录](docs/v0.4.3-validation.md)。
